...
- `autoBind?` - *(optional)* When set to `true`, Rive will automatically look for a default view model and view model instance to bind to the artboard. This is useful when working with data binding features.
...
- `drawingOptions?` - *(optional)* An enum (`DrawOptimizationOptions`) that provides drawing optimization options. This can be used to configure rendering performance optimizations. The default is `DrawOptimizationOptions.DrawOnChanged`.
