...
export interface RiveParameters {
  ...
  semanticsMode?: SemanticMode; // experimental
  semanticsOptions?: RiveSemanticsOptions; // experimental
  enableGPUCanvas?: boolean; // experimental, @rive-app/webgl2 only
...
- `semanticsMode` - *(optional, experimental)* Whether to build an accessible DOM overlay for the graphic's [semantics](/runtimes/web/semantics). Either `SemanticMode.Disabled` or `SemanticMode.Enabled`. Defaults to `SemanticMode.Disabled`.
- `semanticsOptions` - *(optional, experimental)* Options for the accessibility overlay. See [Semantics](/runtimes/web/semantics).
  - `riveCanvasLabel` - *(optional)* The `aria-label` applied to the overlay's container element. Defaults to `"Rive animation"`.
- `enableGPUCanvas` - *(optional, experimental)* Render GPU Canvas content — 3D and shaders — on this instance. `@rive-app/webgl2` only, and cannot be combined with `useOffscreenRenderer: true`. Ignored when you pass a `riveFile`, which carries its own mode. False by default. See [GPU Canvas](/runtimes/web/gpu-canvas).

<Warning>
  `semanticsMode` and `semanticsOptions` are experimental and may encounter breaking behavior changes without a major version bump.
</Warning>
...
interface RiveFileParameters {
  ...
  enableRiveAssetCDN?: boolean;
  enablePerfMarks?: boolean;
  enableGPUCanvas?: boolean; // experimental
  onLoad?: EventCallback;
  onLoadError?: EventCallback;
}
```
...
`enableGPUCanvas` is *(optional, experimental)*. A file's rendering mode is fixed at import and has no setter, so set it here and re-import the file to change it. A GPU Canvas file also cannot be shared across `Rive` instances. See [GPU Canvas](/runtimes/web/gpu-canvas).
