...
You only need to use this function if you are using Rive's [low-level APIs](./low-level-api-usage) to build the render loop manually and are integrating Rive into your existing `requestAnimationFrame()` loop
