...
They share the same high-level API, so switching packages does not require changing how you create a `new Rive({...})` instance. The only difference is the renderer used.
...
- Great fit for simpler vector/raster animations
