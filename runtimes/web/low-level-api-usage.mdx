...
[See here](/runtimes/layout) for options for `Fit` and `Alignment`. For the latter two parameters, provide an axis-aligned bounding box (AABB). See the snippet below for an example of the `align()` API.
...
See more on usage in the [Rive Parameters](./rive-parameters) doc.