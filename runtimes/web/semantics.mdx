...
The semantic overlay's container element is a `role="region"` landmark. Use the `semanticsOptions.riveCanvasLabel` parameter when instancing Rive to give it an `aria-label` that describes what the graphic is, so screen reader users know what they are entering.
...
The overlay wires keyboard interaction to the [semantic actions](/editor/accessibility/semantics#actions) you listen for in the editor. Add a **Semantic Action** listener in your state machine to respond to them.

| Browser Interaction | Semantic Action fired |
| --- | --- |
| **Enter** or **Space** on a button, checkbox, switch, tab, or radio button | Tap |
| **Right** / **Up** arrow on a slider | Increase |
| **Left** / **Down** arrow on a slider | Decrease |
| Arrow keys within a tab list or radio group | Moves focus to the next member and fires Tap |
