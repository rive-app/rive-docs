...
import { ExamplesCards } from '/snippets/examples-cards.jsx';
...
<Tabs>
    ...
    <Tab title="Web">
        ...
        <ExamplesCards examples={['cachingARiveFile']} runtime="web" />


        ```javascript
        import { Rive, Layout, Fit, RiveFile } from "@rive-app/webgl2";

        const riveInstances: Rive[] = [];
        ...
        // Set up each instance of of the loaded Rive file
        function setupRiveInstance(loadedRiveFile, canvasId, component, stateMachine) {
            const canvas = document.getElementById(canvasId) as HTMLCanvasElement;
            if (!canvas) return;

            const riveInstance = new Rive({
                // Instead of using src, we'll use the loadedRiveFile
