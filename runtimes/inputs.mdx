...
<Note>
  Inputs can also be set on nested artboards at runtime, see [Nested
...
You can control the inputs of [Nested Artboards](/editor/fundamentals/components) at runtime. These inputs are not on the main artboard but on a nested artboard. To set a nested input, you need to know the **path** where the input exists at an artboard level.
