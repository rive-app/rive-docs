...
Rive’s Layout feature lets you design resizable artboards with built-in responsive behavior, configured directly in the graphic. Just set a **Fit** to type **Layout** at runtime and the artboard will resize automatically. Optionally, provide a **Layout Scale Factor** to further adjust the scale of the content.

For more Editor information and how to configure your graphic, see [Layouts Overview](/editor/layouts/layouts-overview).

<Note>
  - The **Alignment** property will not have an effect when using a **Fit** of
  type **Layout**.
 - The **LayoutScaleFactor** property only applies to a **Fit** of type **Layout**.
...
<Tabs>
  <Tab title="Web">
    Use the `Layout` object to configure `Fit` and `Alignment`. See [Fit](#fit) and [Alignment](#alignment) for all enum options.
  ...
  <Tab title="React Native">
    <Tabs>
      <Tab title="New Runtime (Recommended)">
        Set layout attributes for `Fit` and `Alignment` on the `RiveView` component directly. See [Fit](#fit) and [Alignment](#alignment) for all enum options.
      ...
      <Tab title="Legacy Runtime">
        Set layout attributes for `Fit` and `Alignment` on the `Rive` component directly. See [Fit](#fit) and [Alignment](#alignment) for all enum options.
