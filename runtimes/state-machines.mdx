...
description: 'Playing the state machines'
---

For more information on designing and building state machines in Rive, please refer to: [State Machine](../editor/state-machine).

Rive's state machines provide a way to combine a set of animation states and manage the transition between them that can be programmatically controlled with [Data Binding](./data-binding) (recommended) and [Inputs](./inputs).
