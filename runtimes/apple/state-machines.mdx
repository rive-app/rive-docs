...
<Tabs>
  <Tab title={"{{runtimeCurrentNameApple}}"}>
    ...
    Note that these are all async throwing functions marked as `@MainActor`. State machine creation also binds authored main and global view model defaults when available. Binding can throw `StateMachineError`; artboard lookup failures throw `ArtboardError`. To supply instances you retain, use `createStateMachine("StateMachine", binding: instance)` and the optional globals builder described in [Data Binding](/runtimes/apple/data-binding#global-view-models).
