...
Use `Rive(file:)` when authored defaults are sufficient and you do not need to access their properties. For programmatic access, replace the legacy auto-bind callback with explicit instance creation and retain the instance in your app's model:
...
Create these components from a `@MainActor` async context. Keep the instance reference for later property access and observation. For global bindings, see [Global View Models](/runtimes/apple/data-binding#global-view-models).

#### Updating existing concurrency runtime code

The global view model update also changes APIs in the new concurrency runtime:

| Previous API | Replacement |
| --- | --- |
| `Rive(file: file, dataBind: .auto)` | `Rive(file: file)` for authored defaults, or explicit instance creation for property access |
| `Rive(file: file, dataBind: .instance(instance))` | `artboard.createStateMachine(binding: instance)`, then pass that artboard and state machine to `Rive` |
| `Rive(..., dataBind: .none)` | Pass the configured artboard and state machine to `Rive` without `dataBind` |
| `rive.viewModelInstance` | Retain the instance you create and bind |
| `stateMachine.bindViewModelInstance(instance)` | `try await stateMachine.bindViewModelInstances(main: instance)` |
| `instance.dirtyStream()` for redraws | Automatic `RiveController` observation of bound instances |

<Warning>
Calls such as `Rive(file:)` can continue compiling while changing behavior: the new initializers leave the deprecated `viewModelInstance` getter `nil`. Neither `Rive` nor `StateMachine` exposes getters for automatically bound instances. Removing `dataBind:` alone is insufficient if your code accesses that getter.
</Warning>

`Rive(file:)` and `Rive(file:artboard:)` create a state machine and bind authored defaults. `Rive(file:artboard:stateMachine:)` uses the supplied state machine without rebinding. State machine creation already binds available defaults, so omitting `dataBind` is not a way to disable that initial binding. Likewise, deprecated `.none` only skips binding in the legacy initializer; it does not undo existing bindings.

For a temporary compatibility bridge, explicitly select the deprecated initializer:

```swift
let rive = try await Rive(file: file, dataBind: .auto)
let instance = rive.viewModelInstance // Legacy-created main instance, when available.
```

`DataBind`, this initializer, and the getter are all deprecated. The getter is an initialization snapshot: later rebinding does not update it. Prefer the explicit setup above for new code.

#### Setting up instances for your app

Create the instances you need, bind them, and keep their references in your app's model. This example uses a default main instance and a "Light" instance from the global "Theme" view model. Run setup in a `@MainActor` async context, using the same file and worker for all components.
...
Keep `rive`, `main`, and `theme`. Set any initial property values before presenting the view. See [Global View Models](/runtimes/apple/data-binding#global-view-models) for replacing bindings and switching themes.

#### Optional auto-bind helper

For default instances, add this extension to your app. It returns `Rive` together with the main and global instances:
...
This is an app-owned helper, not a runtime API. It requires a default main instance and a default instance for each global; missing defaults throw. Without globals, the dictionary is empty.

Call it from a `@MainActor` async context and retain `autoBound`:
...
If you replace a binding later, update your stored instance reference after binding succeeds.
