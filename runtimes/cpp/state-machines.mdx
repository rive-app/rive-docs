...
bool   currentOpen     = isOpen->value();
float  currentProgress = progress->value();
```
...
`StateMachineInstance` (downcast from `Scene` if you need this):

```cpp
auto* sm = static_cast<StateMachineInstance*>(scene.get());
