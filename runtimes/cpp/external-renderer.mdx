...
core runtime is renderer-agnostic — anything you pass to `Scene::draw` only
has to implement two interfaces:
...
prefer the [first-party renderers](/runtimes/cpp/renderers) — they're
...
- The non-virtual `decodeFont` / `decodeAudio` helpers fan out to Rive's
  built-in HarfBuzz / miniaudio paths. Override them in your subclass if
  you have your own font shaping or audio engine.

## RenderPath / RenderPaint subclasses

Each is just a typed bag of state for your backend to consume:
...
(Method signatures match `rive/command_path.hpp` and
`rive/shapes/paint/render_paint.hpp` — read those for the full set.)
...
| Project | Uses | Where |
| ------- | ---- | ----- |
| `rive::gpu::RenderContext` | Pixel-local-storage GPU renderer | `packages/runtime/renderer/` |
| `rive_skia` | Skia | (deprecated, but a reference implementation) |
| Flutter runtime | Flutter Canvas | `packages/runtime_flutter` |
| Unity runtime  | Unity GraphicsBuffer / Mesh | `packages/runtime_unity` |
| Unreal runtime | Unreal RHI | `packages/runtime_unreal` |

The Skia and Flutter implementations are the most direct templates for a
"forward to an existing 2D engine" `Renderer`.
