...
<Tabs>
    ...
    Pass `async: true` to create the instance via the async runtime APIs, off the JS thread. The hook returns `{ instance, isLoading, error }` — the instance is not available on the first render, so gate rendering on it:
