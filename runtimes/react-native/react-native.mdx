...
<Tabs>
  <Tab title="New Runtime (Recommended)">
    ...
    You can find the full quick start example [here](https://github.com/rive-app/rive-nitro-react-native/blob/main/example/src/pages/QuickStart.tsx).
    <Steps>
      ...
      <Step title="Config">
      These imports and styles are needed for the following steps.
      ...
      <Step title="Rive File and Component">
        The `file` prop is the only requirement for a `RiveView` component.

        Load a **riv** file using the `useRiveFile` hook. This value can be cached and used on multiple components, see [caching a Rive file](/runtimes/caching-a-rive-file).
      ...
      <Step title="Layout">
        Configure the layout of the graphic.

        For this example, we'll set the `fit` to `Layout`, which will automatically resize the artboard to match the view size. This works for responsive Rive graphics using [Layouts](/editor/layouts/layouts-overview).
        ...
        See the [runtime layout documentation](/runtimes/layout) for more info.

      </Step>
      <Step title="View Reference">
      You can get/set the Rive view reference with the `useRive()` hook.
      ...
      <Step title="Data binding">
        We recommend manually creating the view model instance with the `useViewModelInstance` hook and passing it to the view.

        This makes it easy to set initial property values in the `onInit` callback before the view is loaded, and decouples the `ViewModelInstance` from the `RiveView`.
        ...
        Make use of the view model property hooks to update and listen to property changes.
    ...
    **Key Properties:**
    ...
    <ResponseField name="hybridRef" type="HybridView">
      Set the view reference, loaded via `useRive`.
    </ResponseField>
    ...
    <ResponseField name="layoutScaleFactor" type="double" default="-1">
      The scale factor to apply to the Rive graphic when using `Fit.Layout`. The default value uses the device DPI as a scale factor.
      <Note>This property has no effect for any other `Fit` type.</Note>
    </ResponseField>
    <ResponseField name="artboardName" type="String">
      The name of artboard to display.

      _If not set the default artboard will be used, as set in the Editor._
    </ResponseField>
    <ResponseField name="stateMachineName" type="String">
      The name of the state machine to play.

      _If not set the default state machine will be used, as set in the Editor._
    </ResponseField>
    ...
    Hook for loading Rive files from various sources:
    ...
    Hook to access the Rive view ref after the view is loaded:
