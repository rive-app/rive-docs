...
Image properties let you bind image assets directly, replacing the legacy asset loader system and referenced assets. Create image assets by decoding image data through a Rive factory, then set them the value of image properties.

<Tabs>
    <Tab title="Apple">
        <Note>On iOS, you do not have to directly reference a factory to create an image asset. Instead, you can use new `RiveImageAsset` initializers.</Note>
...
List properties enable you to work with a dynamic set of view model instances at runtime. A list property can contain a mix of different view model types. With list properties, you can:
