...
<Tabs>
  ...
  <Tab title="Android">
    <Tabs>
      <Tab title="Compose">
        Auto-binding does not exist in the Compose API due to the nature of composables. Since they are functions, it is difficult to get values out of them as compared to classes. We could use callbacks, but that would require a null placeholder to remember the value before it has fired, and would complicate setting initial values before first render. The overhead is no better than supplying the instance directly.
