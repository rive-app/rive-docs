...
Listeners are a way of adding observability to your data binding properties. Using listeners, you will be notified when your property changes are applied by a state machine, whether a new value has been explicitly set or if the state machine itself has caused a change. Adding a listener to trigger properties is a new way to notify you that some event has occurred.
