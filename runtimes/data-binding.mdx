...
Image properties let you set and replace images at runtime, where each image is managed separately. For example, you can make an avatar creator and dynamically change the avatar's features, such as a hat, by setting the value of a view model's image property. You can add further customization by having additional image properties for features like hair, eyes, and more.
...
A single list property can include different view model types, with each view model tied to its own Nested Artboard, making it easy to popular a list with a varity of Nested Artboards.
