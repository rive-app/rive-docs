...
<Warning>Data Binding is in early access and is not yet available for production</Warning>
...
<Note>The initial values of the instance are not applied to their bound elements until the artboard is advanced.</Note>
...
View model instances have mutable properties. References to these properties can be retrieved by name. They have get, set, and observe operations. Getting or observing the value will retrieve the latest value set on that properties binding, as of the last artboard advance. Setting the value will update the value and all of its bound elements. Note that trigger properties can only be set.

<Note>After setting a property's value, the changes will not apply to their bound elements until the artboard is advanced.</Note>
...
View models can have properties of type view model, allowing for arbitrary nesting. We need a way to access properties on nested view models. This is done through a path parameter, which is similar to a URI in that it is a forward slash delimited list of property names ending in the name of the property of interest.
...
<Tabs>
    <Tab title="Android">
        ...
        val nestedNumber = vmi.getNumberProperty("My Nested View Model/My Nested Number")
        ```
    </Tab>
</Tabs>
