...
A project can be a single `.rml` file or several. The [Rive CLI](/cli/overview) compiles them together into one `.riv` file for use with the Rive Runtimes, or a `.rev` file that you can open and continue editing in the Rive Editor.
...
A fragment never declares a `Backboard`. What the Editor keeps there, such as the default artboard and the publish settings, is project configuration, so it lives in [rive.yaml](/cli/reference/project-config) instead. The build creates the `Backboard` from those keys. Only a bundle carries a `Backboard` element.
...
A project can hold any number of `.rml` files, in any folders, and they compile as one
document. Split by whatever reads well, such as artboards in one file and view models
in another. Each file keeps its own `Rive` root with `kind="fragment"`.

Smaller files also mean less for a [coding agent](/cli/agents) to read when it only
needs to change one part of the scene.

```text
myproject/
  rive.yaml
  scene.rml            <Rive version="1" kind="fragment">
  data/
    view_models.rml    <Rive version="1" kind="fragment">
```

An id declared in one file can be referenced from another, ids may not repeat across
files, and a problem is reported against the file it is in.

<Note>
Files compile in path order, and that order decides how artboards land in the `.riv`.
Name the default artboard with `main` in [rive.yaml](/cli/reference/project-config)
rather than relying on a filename that sorts first.
</Note>

Run `rive samples` and pick `rml_split` for a working example.
