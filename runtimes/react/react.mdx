...
<Steps>
  ...
  <Step title="Render the Rive component">
    ...
    - **Rive component** - the quickest way to embed a Rive file, but with limited runtime control.
    - **Hooks** - a React-friendly approach for controlling Rive from React state, props, and events.
    - **Imperative runtime** - a direct runtime approach where you create and manage the Rive instance yourself, usually inside `useEffect`.
  ...
  <Step title="Using the useRive hook">
    ...
    Keep in mind that the canvas size depends on the container it's placed within. Initially, this is 0x0. Either pass a `className` to `RiveComponent` or wrap `RiveComponent` with an appropriately sized container.
