...
<Info>
  Rive runtimes are open source. View the source in the Rive React [GitHub repository](https://github.com/rive-app/rive-react).
</Info>
...
The following parameters are specific to `useRive`, or behave differently in React than they do in the web runtime:

- `enableGPUCanvas` - *(optional, experimental)* Render GPU Canvas content — 3D and shaders — on this instance. `@rive-app/react-webgl2` only. Turning it on flips the `useOffscreenRenderer` default to `false`, since the two cannot be combined. False by default. See [GPU Canvas](/runtimes/react/gpu-canvas).
...
- `assetLoader?` - *(optional)* Callback to load assets. Full patterns and examples in the [loading assets](/runtimes/react/loading-assets) guide.
- `enableRiveAssetCDN?` - *(optional)* Allow the runtime to automatically load assets hosted in Rive's CDN. Enabled by default.
- `enablePerfMarks?` - *(optional)* Emits `performance.mark` / `performance.measure` entries for key Rive startup events for performance profiling purposes. False by default.
- `enableGPUCanvas?` - *(optional, experimental)* Import the file for GPU Canvas content — 3D and shaders. `@rive-app/react-webgl2` only at this time.
...
- `useOffscreenRenderer` - *(optional)* Share a single WebGL context across canvases. Only relevant for `@rive-app/react-webgl2`. Defaults to `true`, unless `enableGPUCanvas` is set, which flips the default to `false`. However, if this parameter is explicitly set, the runtime will honor this flag and ignore the `enableGPUCanvas` one.
...
- `enableGPUCanvas` - *(optional, experimental)* Render GPU Canvas content — 3D and shaders — on this instance. `@rive-app/react-webgl2` only at this time. Turning it on flips the `useOffscreenRenderer` default to `false` if that flag is not set. See [GPU Canvas](/runtimes/react/gpu-canvas).
