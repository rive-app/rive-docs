...
<Note>
Note that you can only use the `assetLoader` callback with the `useRive` and `useRiveFile` hooks, and not the default-exported `<Rive />` component from the React runtime
</Note>
