...
<Note>
  Text runs can also be updated on nested artboards at runtime, see [Read/Update
...
It's possible to set nested text runs at runtime—text that is not on the main artboard but on a [Nested Artboard](/editor/fundamentals/components). To set a nested text run, you'll need to take note of the path where the input exists at an artboard level.
