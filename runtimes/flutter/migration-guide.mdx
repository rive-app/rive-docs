...
`SharedTexturePainter` adds a `riveFactory` getter (default `null`) for the same reason on the `RivePanel` paint pass. Classes that `implements SharedTexturePainter` must add the getter; classes that `extends` it inherit the default.
