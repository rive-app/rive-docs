...
Assets referenced by an excluded artboard may still be included in the `.riv`. To exclude an asset from the runtime file, set its export behavior to **Prevent Export**.
