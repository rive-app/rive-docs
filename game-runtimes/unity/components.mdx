...
The Rive Unity package includes components to help you integrate Rive into your project quickly and easily. These components are a high-level abstraction over the [low-level API](./fundamentals), handling rendering and pointer input, and providing a unified way of working with Rive across render pipelines.
...
<Warning>
 A Rive Panel simply handles drawing the graphics to a texture. You'll need to use the Rive Panel **with** a [Panel Renderer](./components#panel-renderers) to display the texture.
</Warning>
