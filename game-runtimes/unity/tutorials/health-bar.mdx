---
title: "Drive a health bar with data binding in Unity"
description: "Learn how to connect Unity gameplay data to a Rive health bar using view model instance properties."
---

This tutorial walks through connecting a Rive health bar file to Unity using **data binding**. In this setup, your Unity code updates a single number (`health`) while the Rive file handles the visuals (e.g. color changes, and low-health warnings).
...
### Step 4.3: Wait for the widget to load and grab the view model instance properties
...
// Rive fires events when properties change or triggers are fired. So we listen to Rive as the source of change events and invoke our UnityEvents so that other scripts can react to the changes.
...
The key idea here is that your Unity code only updates data. Because the Rive file handles the visual representation, you can change the health bar's appearance in the Rive Editor as much as you want without having to update or write any additional code.
