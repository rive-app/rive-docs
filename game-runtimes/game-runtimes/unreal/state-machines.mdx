...
They are not driven by direct imperative inputs.
...
**State Machines** no longer rely on direct input setters.

Instead, they consume values from a bound **ViewModel**.
...
Trigger properties replace legacy event-style transitions.
...
- Expecting legacy input setters to drive transitions
