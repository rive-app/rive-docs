...
- **Binding Targets**: Instances can be bound to:
  - `URiveArtboard` – sets the data context for a Rive artboard.
  - `FRiveStateMachine` – synchronizes ViewModel data with state machine inputs and outputs. Currently, binding to a `URiveArtboard` automatically bind to the state machine.
...
- Property wrappers expose **typed getters/setters** in both C\+\+ and Blueprints.
- ViewModelInstances can contain **nested ViewModels**.
