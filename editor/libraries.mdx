...
<Note>
  When you add a view model from a library to another file, Rive creates a unique instance of that view model in the Assets panel. Its property values are independent of the library's defaults, even if the instance isn't used on an artboard.
