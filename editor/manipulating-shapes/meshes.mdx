...
<Steps>
  <Step>
    Select an image on the Stage or Hierarchy, then go to the **Deform** section in the Inspector.
  </Step>
  <Step>
...
Enter Mesh Editing Mode by clicking the **V** key or by deselecting **Edit Contour**.
...
Adjust the Detail, Concavity, Uniform, Alpha, and Padding settings to control how the generated contour follows the image.

<VideoEmbed src="https://static.rive.app/video/auto-trace.mp4" />

### Editing Contours

Use **Edit Contour** to adjust the existing contour. The Mesh Pen lets you add new vertices or move existing vertices without warping the image.
...
Forced edges are edges you add manually between mesh vertices.

When using the Mesh Pen, click continuously to create connected vertices with forced edges. Press **Esc** between clicks to create vertices without forcing an edge between them.

<VideoEmbed src="https://static.rive.app/video/forced-edges.mp4" />

Forced edges are shown as thicker lines in the mesh.

Use forced edges when you need more control over the mesh structure, such as defining an important seam, crease, corner, or area where you want the mesh triangles to follow a specific path.
