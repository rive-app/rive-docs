---
title: 'Styles'
description: 'Layouts can have styles applied to directly them'
---
...
When a Layout has a background and/or foreground fill or stroke applied, a blend mode can be applied to the Layout that affects overlapping objects in 2 ways:

1. If a style is applied to the background, it will apply the blend to any objects behind the Layout itself.
2. If a style is applied to the foreground, it will apply the blend to all of the Layout's children, the Layout's background AND any objects behind the Layout itself. 