...
- **Properties** determine what changes (scale, position, rotation, opacity, follow path etc.).
- The **Range** determines which glyphs, words, or lines are affected.

A single modifier group can affect multiple properties all sharing the same range.
...
Select a text object, then click the `+` button in the **Modifier Group**. Each property has its own settings that can be adjusted and animated.

![Adding Properties](/images/editor/text/add-modifier-property.gif)

#### Opacity

The Opacity value controls the opacity of text within the modifier's range.
...
Use the offset value to move the range along the text. Try animating the offset value to create wipe effects along a text value. For example, a scale modifier can animate the offset to make individual glyphs scale up and back down again.
