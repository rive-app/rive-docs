...
return function(): Drawing<TrackPointers>
  return {
    init = init,
    advance = advance,
    draw = draw,
...
If you need to listen for Pointer Events in your [instantiated artboards](/scripting/script-types/node-scripts#instanting-components),
you'll need to pass them down manually.
...
```lua
-- Listen for Pointer Events in your main script
function handlePointerDown(self: MyScript, event: PointerEvent)
  ...
  for _, enemy in self.enemies do
    ...
    -- Pass the pointer event to the instantiated artboard
