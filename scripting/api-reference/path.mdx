...
Do not mutate or reset a [Path](/scripting/api-reference/path) in the same frame after drawing it.
...
Returns a [ContourMeasure](/scripting/api-reference/contour-measure) for the first contour in the path. A contour is a
...
[ContourMeasure](/scripting/api-reference/contour-measure) to iterate through subsequent contours. Returns `nil` if the
