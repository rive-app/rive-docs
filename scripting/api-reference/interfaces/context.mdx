...
{/* function gpuCanvas(self, desc: {
        width: number,
        height: number,
        --- Number of MSAA samples (1, 2, 4, or 8). Default: 1 (no MSAA).
        --- Query context:features().maxSamples for the backend's maximum.
        sampleCount: number?,
    }): GPUCanvas */}
<div class="signature">
```lua
gpuCanvas(desc: {width: number, height: number, --- Number of MSAA samples(1, 2, 4, or 8). Default: 1(no MSAA). --- Query context: features().maxSamples for the backend's maximum. sampleCount: number?,}) -> GPUCanvas
```
</div>
...
Use this at init time to create [GPUTexture](/scripting/api-reference/gpu/g-p-u-texture) and [GPUPipeline](/scripting/api-reference/gpu/g-p-u-pipeline)
