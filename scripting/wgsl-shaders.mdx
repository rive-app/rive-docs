...
Call `srcCanvas.image:view()` to obtain a `GPUTextureView`, then add that view to the shader's `GPUBindGroup`. Redraw the artboard to the canvas as it changes to keep the texture up to date.
