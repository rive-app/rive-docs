...
![Image](https://ucarecdn.com/2af0d20a-003f-4145-9e36-34cd91f7d51a/)
...
![Image](https://ucarecdn.com/fc2257a9-0fb8-4ac0-aa9f-5b4dd5adb70a/)

*Tilemap - 9 tiles visible*
...
![Image](https://ucarecdn.com/23614552-6560-4358-997c-fec0c4c8cee2/)

*Tilemap - all 64 tiles visible*
...
<Steps>
  <Step>Create an AABBTree, with a generic **SceneObject** type. This is used to store information about all of the scene objects and to efficiently query for objects that are within a provided AABB space.</Step>
  <Step>Create all the tile objects, which store information about the tile, artboard and position for easy reference. This could be any object that stores the needed information to render your scene.</Step>
  <Step>Add all the objects to the AABBTree using the `createProxy` method. This takes in an AABB and a SceneObject.</Step>
  <Step>Create a view transform (Mat2D) that represents the visible area of the world (camera) - depending on the camera zoom and translation.</Step>
  <Step>Create an AABB that represents the camera's view according to the window size. This is used to determine which tiles are visible on screen.</Step>
  <Step>Query the AABB Tree to find all the objects who's bounds intersect with the `cameraAABB`. These will be the tiles that are visible on screen. Once you have these, you can draw them to the renderer.</Step>
</Steps>

Running the above code you'll see a print that indicates how many tiles are currently visible on screen. This will print a different result if you resize the window, zoom in or out, or moving the camera's translation around. Try updating the `cameraZoom` and `cameraTranslation` and performing a hot reload. Or add [keyboard and mouse interactions](/rive-gamekit/tutorials/adding-keyboard-and-mouse-interaction) to move the camera.