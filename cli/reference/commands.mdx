...
| `--publish` | Write a signed `.riv`. Needs `rive login`, and watermarks the output until the project is bound to a Rive file. Exit 1 on errors |
...
is frames at 60fps; `1s` or `250ms` is animation time, stepped in 1/60s frames with
one shorter frame for any remainder.
...
Values are digits only. A sign, whitespace, trailing text, or anything above a
32-bit unsigned integer is rejected.

<Note>
`--advance` replaces `--frame`. A bare `--advance=N` means what `--frame=N` meant, so
`--frame=20` becomes `--advance=20`. Passing `--frame` or `--wait` is an error that
names `--advance`.

`--bench` takes its own frame count and refuses `--advance`.
</Note>
...
and unparseable values on flags it does, then builds and exits `0`. `--bogus` and
`--viewport=abc` both succeed silently. A green exit is not proof
your flags landed.
</Warning>
