...
| `.rml` | The scene. With more than one, the first by project-relative path wins and the rest are dropped with a `multiple .rml files found` line, so `scene.rml` loses to `other.rml` and to `a/z.rml`. Use `exclude` to pin the one you meant |
...
the scan tidy, and to keep a second `.rml` from being picked up.
