...
The CLI ships seven example projects. `rive samples` opens a picker and copies the
...
```bash
rive myproject --screenshot=out.png                    # frame 0
rive myproject --screenshot=out.png --frame=60         # one second in, at 60fps
...
rive myproject --screenshot=on.png    --pointer=click@120,60 --frame=20
```
...
rive myproject --screenshot=on.png  --pointer=click@120,60 --frame=20
rive myproject --screenshot=off.png --pointer=click@120,60 --pointer=move@400,400 \
                                    --pointer=click@120,60 --frame=20
```

Put a `move` between two clicks on the same control, as above. Back-to-back clicks
can both act on the old value.
</UseCase>
...
A fixed-size artboard renders at its authored size anchored top-left, so a smaller
viewport crops it. That is the flag working, not a layout bug.
