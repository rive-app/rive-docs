...
The CLI ships a number of example projects. `rive samples` opens a picker and copies the
...
| `text_input` | Editable text fields with the TextInput component: placeholder, obscured entry, Tab traversal |
| `keyboard_menu` | A keyboard-driven menu with no script: named key filters, view model triggers driving a state machine |
| `cursor_demo` | Asking the window for an OS cursor: hand, resize, I-beam |
| `integrated_titlebar` | An app shell under the macOS traffic lights: integrated title bar, draggable header, resizable sidebar |
